...
const Authors = () => {
      ...
      setLoading(true)
         ...
         const todos =   await API.graphql({
            ...
            const booksWithImages = await Promise.all(res.map(async (book) => {
              const imageUrl = await Storage.get(book.authorImage);
  ...
  return (
    <div className='w-full justify-center bg-cart-img min-h-screen gap-4 flex-wrap flex pt-28 px-5' >
    {
      data?.map((book)=>{
        return(
        

              <div key = {book.id} className="card shadow-lg border rounded-xl ">
        <div className="card-info">
        <div className="card-avatar">
          <img src={book.bookImage} alt="authorImage" className = 'w-full h-full rounded-full ' />
        </div>
        <div className="card-title">{book.author}</div>
        <div className="card-subtitle">{book.email}</div>
        </div>
